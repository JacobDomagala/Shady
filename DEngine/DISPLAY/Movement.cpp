...
glm::vec3 Movement::GetPosition() const
{
	return position;
}
glm::mat4 Movement::GetWorldToViewMatrix() const
{
	return viewMatrix;
}
...
void Movement::SetCameraMode(int mode){
	if (mode == FLY)
		flyMode = true;
	else if (mode == STATIC)
		flyMode = false;
...
void Movement::IsOtherKeyPressed(int vKey){
...
glm::vec3 Movement::GetLightPosition(){
	return lightPos;
}
void Movement::SetLightPosition(glm::vec3 lightPos){
	this->lightPos = lightPos;
}
float Movement::GetDelta(){
	return deltaTime;
}
void Movement::SetCamera(glm::vec3 cameraPosition, glm::vec3 viewDirection){
