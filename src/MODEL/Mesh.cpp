...
void
Mesh::Draw(GLuint programID)
{
  if (textures.size()) {
    for (GLuint i = 0; i < textures.size(); i++) { textures[i].Use(programID, i); }
  }
  // Draw mesh
...
Mesh::SetupMesh()
{
  ...
  glVertexAttribPointer(
    3, 3, GL_FLOAT, GL_FALSE, sizeof(Vertex), (GLvoid *)offsetof(Vertex, tangent));
