#include "Model.h"
Model::Model()
  : translateValue(0.0, 0.0, 0.0), rotateValue(1.0, 1.0, 1.0), scaleValue(1.0, 1.0, 1.0),
    rotateAngle(0.0)
{}
void
...
Model::Draw(shady::app::Window *window, Camera camera, Light *lights, Shader shader)
{
  ...
  modelMatrix =
    glm::translate(translateValue) * glm::rotate(rotateAngle, rotateValue) * glm::scale(scaleValue);

  glUniformMatrix4fv(glGetUniformLocation(shader.programID, "projectionMatrix"),
    1,
    GL_FALSE,
    glm::value_ptr(projectionMatrix));
  glUniformMatrix4fv(
    glGetUniformLocation(shader.programID, "viewMatrix"), 1, GL_FALSE, glm::value_ptr(viewMatrix));
  glUniformMatrix4fv(glGetUniformLocation(shader.programID, "modelMatrix"),
    1,
    GL_FALSE,
  ...
  for (GLuint i = 0; i < meshes.size(); i++) { meshes[i].Draw(shader.programID); }
}
...
Model::LoadMaterialTextures(aiMaterial *mat, aiTextureType type, char *typeName)
{
  ...
  for (GLuint i = 0; i < mat->GetTextureCount(type); i++) {
    ...
    // Check if texture was loaded before and if so, continue to next iteration: skip loading a new
    // texture
...
Model::CreateCube(int size)
{
  ...
  Vertex verticies[] = { vec3(0.0f, 0.0f, 0.0f),
    vec3(0.0f, 0.0f, -1.0f),
    vec2(1.0f, 1.0f),
    vec3(),
    vec3(size, 0.0f, 0.0f),
  ...
  for (unsigned int i = 0; i < 24; i++) { cube.vertices.push_back(verticies[i]); }
  ...
  GLushort indieces[] = { 0,
    ...
    22,
    23 };
  for (unsigned int i = 0; i < 36; i++) { cube.indices.push_back(indieces[i]); }
  Texture tmp[2];
